      ...
      <div className="theme-card fade-in relative z-10 flex max-h-[calc(100dvh-2rem)] w-full max-w-2xl flex-col overflow-hidden rounded-3xl">
        ...
        <div className="relative z-10 -mt-9 shrink-0 px-5">
          ...
          <input
            ...
            className="display-type mt-1.5 w-full bg-transparent text-2xl font-light text-moon outline-none placeholder:text-moon/30"
          />
          {place?.name ? (
            <p className="mt-0.5 flex items-center gap-1.5 truncate text-[0.8125rem] text-moon/45">
        ...
        <div className="glass-scroll mt-3 min-h-0 flex-1 space-y-4 overflow-y-auto px-5 pb-1">
          ...
          <div className="grid grid-cols-2 gap-3">
            <Field label="Starts">
              <input
                ...
                className="editable-date clock-figures w-full rounded-xl border border-white/12 bg-white/8 px-3 py-2 text-sm text-moon outline-none focus:border-accent/40"
              />
            </Field>
            <Field label="Ends">
              <input
            ...
            {place && !searching ? (
              <div className="flex items-center gap-3 rounded-2xl bg-white/[0.05] p-2.5 ring-1 ring-white/10">
                ...
                <div className="min-w-0 flex-1">
                  <p className="truncate text-sm font-medium text-moon">{place.name}</p>
                  <p className="truncate text-[0.8125rem] text-moon/45">{place.address}</p>
                </div>
                {place.rating ? (
                ...
                <div className="flex shrink-0 flex-col gap-1">
                  <button
                    ...
                    className="soft-button rounded-lg px-2.5 py-1 text-[0.8125rem] font-semibold text-moon/75 focus:outline-none"
                  >
                    Change
                  </button>
                  <button
                    ...
                    className="rounded-lg px-2.5 py-1 text-[0.8125rem] font-medium text-moon/35 transition hover:text-rose-300 focus:outline-none"
                  >
                    Clear
                  </button>
                </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <Field label={`Cost${currency ? ` (${currency})` : ''}`}>
              <input
                ...
                placeholder="0"
                aria-label="Cost"
                className="clock-figures w-full rounded-xl border border-white/12 bg-white/8 px-3 py-2 text-sm text-moon outline-none placeholder:text-moon/25 focus:border-accent/40"
              />
            </Field>
            <Field label="Booking">
              <button
                ...
                className={[
                  ...
                  item.booked
                    ...
                    : 'border-white/12 bg-white/8 text-moon/55 hover:bg-white/12',
                ].join(' ')}
              >
          ...
          <Field label="Notes">
            <textarea
              ...
              className="glass-scroll w-full resize-none rounded-xl border border-white/12 bg-white/8 px-3 py-2 text-[0.8125rem] leading-6 text-moon/85 outline-none placeholder:text-moon/25 focus:border-accent/40"
            />
          </Field>
        </div>
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className={[
                ...
                item.done ? 'bg-accent/15 text-accent ring-1 ring-accent/25' : 'soft-button text-moon/70',
              ].join(' ')}
            >
            ...
            <button
              type="button"
              onClick={onClose}
              className="rounded-xl bg-white/10 px-4 py-2 text-xs font-semibold text-moon ring-1 ring-white/15 transition hover:bg-white/16 focus:outline-none focus-visible:ring-2 focus-visible:ring-white/50"
            >
              Close
            </button>
          </div>
  ...
  return (
    <div className="relative h-52 shrink-0 overflow-hidden">
      {current ? (
        ...
        >
          <span className="flex flex-col items-center gap-1.5 text-moon/30">
            <ImageOff className="h-6 w-6" aria-hidden="true" />
            <span className="text-[0.75rem] font-medium">No photos yet</span>
          </span>
        </div>
      )}
      ...
      <button
        ...
        onClick={onClose}
        aria-label="Close"
        className="absolute right-3 top-3 grid h-8 w-8 place-items-center rounded-full bg-[#0b1024]/70 text-moon/75 backdrop-blur-md transition hover:text-moon focus:outline-none focus-visible:ring-2 focus-visible:ring-white/50"
      >
      ...
      {count > 1 && (
        <>
          <button
            ...
            className="absolute left-2 top-1/2 grid h-8 w-8 -translate-y-1/2 place-items-center rounded-full bg-[#0b1024]/60 text-moon/80 backdrop-blur-md transition hover:bg-[#0b1024]/85 focus:outline-none"
          >
          ...
          <button
            ...
            className="absolute right-2 top-1/2 grid h-8 w-8 -translate-y-1/2 place-items-center rounded-full bg-[#0b1024]/60 text-moon/80 backdrop-blur-md transition hover:bg-[#0b1024]/85 focus:outline-none"
          >
      ...
      <div className="flex flex-wrap items-center gap-1.5">
        {categories.map((category) => {
          ...
          return (
            <button
              ...
              className={[
                'rounded-full px-3 py-1 text-[0.8125rem] font-semibold transition focus:outline-none focus-visible:ring-2 focus-visible:ring-white/50',
                active ? 'text-moon' : 'soft-button text-moon/55',
              ].join(' ')}
              style={
                active
                  ? {
        ...
        {onAdd && !adding && (
          <button
            ...
            className="grid h-6 w-6 place-items-center rounded-full border border-dashed border-white/25 text-moon/40 transition hover:border-accent/55 hover:text-accent/70 focus:outline-none"
          >
      ...
      {adding && (
        <div className="flex flex-wrap items-center gap-2 rounded-2xl bg-white/[0.04] p-2.5 ring-1 ring-white/10">
          <input
            ...
            className="min-w-0 flex-1 rounded-lg border border-white/12 bg-white/8 px-2.5 py-1.5 text-xs text-moon outline-none placeholder:text-moon/30 focus:border-accent/40"
          />
          ...
          <button
            ...
            onClick={create}
            disabled={!label.trim()}
            className="rounded-lg bg-accent/15 px-3 py-1.5 text-[0.8125rem] font-semibold text-accent ring-1 ring-accent/25 transition hover:bg-accent/22 focus:outline-none disabled:opacity-40"
          >
            Add
          </button>
          <button
            ...
            className="rounded-lg px-2 py-1.5 text-[0.8125rem] font-medium text-moon/45 transition hover:text-moon/80 focus:outline-none"
          >
            Cancel
          </button>
        </div>
